...
CobbDouglasBio.CAA  <- function(fleets, biols, fleets.ctrl, advice, year = 1, season = 1, flnm = 1, stknm = 1, ...){
    ...
    catch.restr <- ifelse(is.null(fleets.ctrl[[flnm]]$restriction[[yr]]), 'landings', fleets.ctrl[[flnm]]$restriction[[yr]])
...
CobbDouglasAge.CAA <- function(fleets, biols, fleets.ctrl, advice, year = 1, season = 1, flnm = 1, stknm = 1,...){
 ...
 #   if(year == 35 & stknm == 'HKE') browser()
    ...
    if(TACOS){
        ...
        QS          <- yr.share*ss.share                                          # [it]
        QS[is.na(QS)] <- 0              
        tac <- ((advice$TAC[st,yr]*QS)[drop=T]*(yrtr_p)) - yrtr_disc # it, add yeartransfer in case it is in place, first we increment in % the quota and then we discount the cuota used in previous year. 
    }
