...
#     then Ftg is min(Fsq,((1-(1/(2020-y)))*Fsq)+((1/(2020-y))*Fmsy)) # don't want to increase F
#     Keep the biomass safeguard, so you can only delay Fmsy when B>Bpa, else F is determined by the B rule
# Paul Dolder (adapting work by Dorleta GarcYYYa)
# Created: 15/15/2015 
# Changed: 
#-------------------------------------------------------------------------------
   ...
   # else if Fadv tq SSB[tac year + 1] > SSB[data year] + (Bpa - SSB[data year])/K, if this leads to and F > Ftarget => Fadv = Farget
    ...
    for(i in 1:iter){
      ...
      if(ssbTACyr < Bpa){
          ...
          if(!is.finite(Cup)) {
          fwd.ctrl2 <- fwdControl(data.frame(year = c(assyrnumb+1, assyrnumb+1),  val = c(ssbobj,NA), quantity = c( 'ssb', 'f'),
                                               min = c(NA, 0.2*Ftg), max  = c(NA, Ftg)))
      ...
      #print(Ftg2)
      # take the min or fsq or the new target, but ensuring fsq is not below Ftg
      # If already below or equal to Ftg, can go straight there (subject to TAC constraint)
      Ftg2<-ifelse(Ftg2<=Ftg,Ftg,min(max(fsq,as.numeric(Ftg)),Ftg2)) 
      ...
      if (!is.finite(Cup)) {
          fwd.ctrl2 <- fwdControl(data.frame(year = c(assyrnumb+1),  val = c(Ftg2), quantity = c('f')))
